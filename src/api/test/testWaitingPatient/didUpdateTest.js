...
export default {
  Mutation: {
    didUpdateTest: async (_, args, { request, isAuthenticated }) => {
      isAuthenticated(request);
      const { user } = request;
      const { did_id, uniqueId, email } = args;
      try {
        ...
        console.log("did 테스트 변경 실패. didUpdateTest", e);
        throw new Error("did 테스트 변경에 실패하였습니다.");
      }
    },
  },
};
